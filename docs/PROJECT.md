...
| 专业权威词库 | 📋 规划中 | 未来扩展数据来源，提升覆盖面和权威性 |

### 3.3 数据导入流程

```
原始讲义 (Markdown)
    ↓ scripts/import-data.mjs（解析提取）
中间格式 (parsed-data.json)
    ↓ scripts/generate-roots.mjs（生成 TypeScript）
类型安全的数据文件
    ├── content/roots.ts        → RootEntry[]
    ├── content/affixes.ts      → AffixEntry[]
    └── content/words/[root]/   → WordEntry[]（按词根目录组织）
```

所有数据在构建时静态加载，无运行时 API 调用。

### 3.4 数据文件组织

```
content/
├── site.ts          # 站点配置（名称、URL、导航等）
├── roots.ts         # 122 个词根（约 4500 行）
├── affixes.ts       # 41 个词缀（约 500 行）
└── words/           # 单词按词根目录组织
    ├── index.ts     # 聚合所有词根下的单词
    ├── act/index.ts # act 词根的关联单词
    ├── bio/index.ts # bio 词根的关联单词
    └── ...          # 130+ 个子目录
```
...
| 层级 | 技术 | 版本 | 选型理由 |
|------|------|------|----------|
| **框架** | Next.js | 14 | App Router + 静态导出，SEO 友好 |
...
项目采用 Next.js 的 `output: 'export'` 模式，在构建时预渲染所有页面为静态 HTML。这意味着：

- 约 1,300+ 个页面在 `next build` 时全部生成
- 可直接部署到任何 CDN / 静态托管
...
| `/root/[slug]` | 词根详情 | SSG | 词根信息、关联单词表、相关词根 |
| `/word/[slug]` | 单词详情 | SSG | 发音、释义、词素分解、例句、相关词 |
...
lib/content.ts          ← 数据加载工具（getRootBySlug 等）
lib/i18n/               ← 国际化字典
